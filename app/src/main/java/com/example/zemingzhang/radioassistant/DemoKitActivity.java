...
public class DemoKitActivity extends Activity implements Runnable {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// get detached from usb event
		IntentFilter filter = new IntentFilter(ACTION_USB_PERMISSION);
        filter.addAction(ACTION_USB_PERMISSION);
		filter.addAction(UsbManager.ACTION_USB_ACCESSORY_DETACHED);
		registerReceiver(mUsbReceiver, filter);
		Toast.makeText(getApplicationContext(), "DemoKit onCreate",
				Toast.LENGTH_SHORT).show();
		if (getLastNonConfigurationInstance() != null) {
			Toast.makeText(getApplicationContext(), "getLastNonConfigurationInstance not NULL",
					Toast.LENGTH_SHORT).show();
